...
function buildNotation(pool: Record<number, number>, mod: number) {
  const parts = DICE.filter((s) => pool[s]).map((s) => `${pool[s]}d${s}`);
  let n = parts.join(" + ");
  if (mod) n += n ? ` ${mod > 0 ? "+" : "−"} ${Math.abs(mod)}` : `${mod}`;
  return n;
}
...
export function DiceTray({ room }: { room: RoomStore }) {
  ...
  }, [room.room?.id, room.supabase, room.userId, toast]);

  const notation = buildNotation(pool, modifier);
  const diceCount = Object.values(pool).reduce((a, b) => a + b, 0);
  ...
  async function roll() {
    ...
    setRolling(true);
    try {
      const groups = DICE.filter((s) => pool[s]).map((s) => `${pool[s]}d${s}`);
      const raw = await box.roll(groups);
      ...
      const detail: DiceDetail[] = [...bySides.entries()]
        .sort((a, b) => a[0] - b[0])
        .map(([sides, values]) => ({ sides, values }));

      const sum = detail.reduce(
        (a, g) => a + g.values.reduce((x, y) => x + y, 0),
        0,
      );
